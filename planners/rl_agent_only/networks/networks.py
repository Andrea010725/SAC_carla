...
class PPONetwork(Network):
    ...
    def value_predict(self, inputs):
        #return self.value(inputs, training=False)    #原来的
    ...
    def value_network(self, **kwargs):
        ...
        # 原来的 value 输出 (base, exp)
        value = self.value_head(last_layer, **kwargs)
        self.exp_scale = kwargs.get('exponent_scale', self.exp_scale)

        # ==== 新增: TRD head ====
        # 输出 [B, trd_bins+1] 的 TRD 向量
        trd_units = self.trd_bins + 1
        trd_vec = Dense(units=trd_units,
                        activation='linear',
                        name='trd-head')(last_layer)

        # Model 同时输出 value 和 TRD
        return Model(
            list(inputs.values()),
            outputs=[value, trd_vec],
            name='Value-Network'
        )
