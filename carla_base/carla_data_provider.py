...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
	...
	_random_seed = random.randint(1, 99999999)
	_rng = random.RandomState(_random_seed)
	_traffic_manager = None
	_grp = None

	_scenario_end_wp = None
	_scenario_start_wp = None
	_harzard_end_actor = None
	_accident_direction = None
	...
	@staticmethod
	def set_trafficmanager(client):
		"""
		Set the world and world settings
		"""
		print('CarlaDataProvider Set Traffic Manager')
		CarlaDataProvider._traffic_manager = client.get_trafficmanager(8000)
		# CarlaDataProvider._traffic_manager = client.get_trafficmanager(CarlaDataProvider._traffic_manager_port)
		# CarlaDataProvider._traffic_manager = None
	...
	def get_trafficmanager():
		...
		Return world
		"""
		return CarlaDataProvider._traffic_manager
	...
	def request_new_actor(
			...
			attribute_filter=None, tick=True, retry=5
	):
		"""
		...
		if random_location:
			...
			z_offset = 0.5 if 'prop' not in model else 0
		...
		if actor is None:
			if retry > 0:
				sleep(1.5)
				# print("RETRY-WARNING: Cannot spawn actor {} at position {}".format(model, spawn_point.location))
				return CarlaDataProvider.request_new_actor(model, spawn_point, rolename, autopilot, random_location, color, actor_category, attribute_filter, tick, retry - 1)
			else:
	...
	def request_new_batch_actors(model, amount, spawn_points, autopilot=False,
								 ...
								 attribute_filter=None, tick=True, near_ego_loc=None):
		"""
			...
			if random_location:
				if CarlaDataProvider._spawn_index >= len(CarlaDataProvider._spawn_points):
					...
					while True:
						...
						CarlaDataProvider._spawn_index += 1
						if near_ego_loc is not None:
							if spawn_point.location.distance(near_ego_loc) < 50:
								break
							else:
								continue
						else:
	...
	def cleanup():
		...
		CarlaDataProvider._parked_vehicles = []

		CarlaDataProvider._scenario_end_wp = None
		CarlaDataProvider._scenario_start_wp = None
		CarlaDataProvider._harzard_end_actor = None
		CarlaDataProvider._accident_direction = None
		CarlaDataProvider._agent_instance = None