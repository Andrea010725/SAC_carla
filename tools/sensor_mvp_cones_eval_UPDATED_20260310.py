...
def compute_target_lookahead_from_env(carla_env, ego_tf: carla.Transform) -> float:
    """
    Use env target waypoint to keep next_wp_* definition consistent with current SAC/GT pipeline.
    """
    try:
        ...
        if target_wp is not None:
            ...
            dx = float(tgt.x - ego_loc.x)
            dy = float(tgt.y - ego_loc.y)
            d = float(math.hypot(dx, dy))
            if np.isfinite(d) and d > 1e-3:
                return float(np.clip(d, 3.0, 25.0))
    except Exception:
        pass
    step = float(getattr(carla_env, "wp_step_dist", 5.0))
    return float(np.clip(step, 3.0, 25.0))
...
class SensorEstimator:
    ...
    def estimate(
        ...
        try:
            ...
            if target_wp is not None:
                ...
                t_rel_x = float(tdx * float(ego_fwd.x) + tdy * float(ego_fwd.y))
                t_rel_y = float(tdx * ego_left_x + tdy * ego_left_y)
                target_dir_xy = (t_rel_x, t_rel_y)
        except Exception:
            target_dir_xy = None

        lookahead_dist = compute_target_lookahead_from_env(carla_env, ego_tf)
        ...
        meta = {
            ...
            "lane_dir_by_target": float(lane_meta.get("lane_dir_by_target", 0.0)),
        }
    ...
    def _estimate_lane_from_points(
        ...
        target_dir_xy: Optional[Tuple[float, float]],
        ...
        if len(xs) < 2:
            ...
            used_target_fb = 0.0
            if target_dir_xy is not None:
                t_long_fb = float(target_dir_xy[0])
                t_lat_fb = float(target_dir_xy[1])
                if np.isfinite(t_long_fb) and np.isfinite(t_lat_fb):
                    # Keep target waypoint relative position even when lane fit fails.
                    lane_fallback[3] = t_long_fb
                    lane_fallback[4] = t_lat_fb
                    used_target_fb = 1.0
            return lane_fallback, {
                "lane_dir_sign": 0.0,
                "lane_lookahead_used": float(np.clip(lookahead_dist, 3.0, 25.0)),
                "lane_dir_by_target": float(used_target_fb),
            }
        ...
        t_lat = 0.0
        if target_dir_xy is not None:
            t_long, t_lat = float(target_dir_xy[0]), float(target_dir_xy[1])
            if np.isfinite(t_long) and np.isfinite(t_lat):
                used_target = 1.0
                dot_pos = float(dir_vec[0] * t_long + dir_vec[1] * t_lat)
                dot_neg = float((-dir_vec[0]) * t_long + (-dir_vec[1]) * t_lat)
                if dot_neg > dot_pos:
                    dir_sign = -1.0
        # Coarse fallback: assume forward lane direction when target direction is unavailable.
        # This avoids introducing additional instability from velocity-based direction estimates.
        if used_target < 0.5:
            dir_sign = 1.0
        ...
        # Width from semantic/lidar is often biased narrow in cones scene.
        # Use a soft prior (instead of hard clipping to a narrow band) to reduce systematic underestimation.
        lane_width_default = 5.5
        lane_width_prior = 5.5
        lane_width_prior_w = 0.65
        lane_width_min = 2.8
        lane_width_max = 7.5
        if widths:
            lane_width_raw = float(np.median(np.asarray(widths, dtype=np.float32)))
            if (not np.isfinite(lane_width_raw)) or (lane_width_raw < 1.8) or (lane_width_raw > 9.0):
                lane_width = lane_width_default
            else:
                lane_width = float((1.0 - lane_width_prior_w) * lane_width_raw + lane_width_prior_w * lane_width_prior)
        else:
            lane_width = lane_width_default
        lane_width = float(np.clip(lane_width, lane_width_min, lane_width_max))
        next_wp_dx = float(x_eval)
        next_wp_dy = float(a * x_eval_lateral * x_eval_lateral + b * x_eval_lateral + c)
        if used_target > 0.5:
            # Keep heading definition as lane tangent (same as GT semantics).
            # Only use target waypoint to define next_wp relative position.
            next_wp_dx = float(t_long)
            next_wp_dy = float(t_lat)
        ...
        return lane, {
            ...
            "lane_dir_by_target": float(used_target),
        }
...
def main():
        ...
        try:
            ...
            while valid_steps < int(args.steps) and total_env_steps < int(args.max_total_env_steps):
                ...
                while (not done) and valid_steps < int(args.steps) and total_env_steps < int(args.max_total_env_steps):
                    ...
                    lane_dir_by_target_steps += int(est_meta.get("lane_dir_by_target", 0.0) > 0.5)
                    ...
                    # Main obstacle GT is fixed to CARLA world API.
                    obs_gt_k3 = obs_gt_world_k3

                    # SAC-style metrics are normalized from API GT, not from env observation slices.
    ...
    summary = {
        ...
        "lane_dir_by_target_rate": float(lane_dir_by_target_steps / max(1, valid_steps)),
