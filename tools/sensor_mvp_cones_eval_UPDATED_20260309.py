...
class SensorEstimator:
    ...
    def estimate(
        ...
        try:
            ...
            if target_wp is not None:
                ...
                t_rel_x, t_rel_y = carla_env._world_to_ego_frame(tdx, tdy, ego_yaw)
                target_dir_xy = (float(t_rel_x), float(t_rel_y))
        except Exception:
            target_dir_xy = None
    ...
    def _estimate_lane_from_points(
        ...
        if target_dir_xy is not None:
            ...
            t_norm = float(math.hypot(t_long, t_lat))
            if t_norm > 0.3:
                used_target = 1.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="world",
        help="world: world.get_actors-style GT; env_obs: same candidate source as current SAC obstacle obs.",
    )
...
def main():
        ...
        try:
            ...
            while valid_steps < int(args.steps) and total_env_steps < int(args.max_total_env_steps):
                ...
                while (not done) and valid_steps < int(args.steps) and total_env_steps < int(args.max_total_env_steps):
                    ...
                    obs_gt_env_k3 = compute_obstacle_gt_env_obs(
                        carla_env=env,
                        obstacle_k=int(args.obstacle_k),
                        obs_range=float(args.obs_range),
                        front_fwd_min=float(args.front_fwd_min),
                    ...
                    obs_gt_k3 = obs_gt_world_k3 if str(args.obstacle_gt_source) == "world" else obs_gt_env_k3
                    ...
                    obs_abs_env_obs = np.abs(obs_pred_k3 - obs_gt_env_k3)
                    ...
                    obs_errs_env_obs.append(obs_abs_env_obs)
    ...
    obs_mae_env_obs = np.mean(np.stack(obs_errs_env_obs, axis=0), axis=(0, 1))
    ...
    summary = {
        ...
        "map": str(args.map),
        "obstacle_gt_source": str(args.obstacle_gt_source),
        ...
        "obstacle_mae_env_obs": {k: float(v) for k, v in zip(OBS_NAMES, obs_mae_env_obs.tolist())},
