...
SCRIPT_VERSION = "sensor_mvp_api_gt_v20260310"
...
class SensorEstimator:
    ...
    def _estimate_lane_from_points(
        ...
        if len(xs) < 2:
            ...
            if target_dir_xy is not None:
                ...
                if np.isfinite(t_long_fb) and np.isfinite(t_lat_fb):
                    # Keep next waypoint relative position even when lane fitting fails.
        ...
        if used_target > 0.5:
            # Keep heading definition as lane tangent; only replace next waypoint position with target semantics.
