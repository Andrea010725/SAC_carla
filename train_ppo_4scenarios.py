...
def train_ppo():
    print("=" * 70)
    print("PPO训练 - 4场景随机切换版本")
    print("场景池: cones, jaywalker, trimma, construction_lane_change")
    print("=" * 70)
    ...
    if WANDB_AVAILABLE:
        try:
            wandb_run = wandb.init(
                ...
                name=f"ppo-4scenarios-{time.strftime('%Y%m%d-%H%M%S')}",
                config={
                    "algorithm": "PPO",
                    "env": "CARLA",
                    "scenario": "random_4scenarios",
                    "scenario_pool": ["cones", "jaywalker", "trimma", "construction_lane_change"],
                    "policy_lr": 5e-5,
                    "value_lr": 1e-4,
                    "gamma": 0.99,
                    "lambda": 0.95,
                ...
                tags=["ppo", "carla", "4scenarios", "cones", "jaywalker", "trimma", "construction"],
                notes="PPO training with 4 random scenarios: cones, jaywalker, trimma, construction_lane_change"
            )
            print("\n✅ Wandb已初始化")
    ...
    print("\n[场景配置] 4场景随机切换模式:")
    for i, scenario in enumerate(config.scenario_pool, 1):
        print(f"  {i}. {scenario}")
    ...
    config.spawn_min_gap_from_cone = 20.0

    # ===== Jaywalker场景参数 =====
    config.jaywalker_distance = 20.0
    config.jaywalker_speed = 2.0
    config.jaywalker_trigger_distance = 15.0
    config.jaywalker_start_side = "random"

    # ===== Trimma场景参数 =====
    config.front_vehicle_distance = 18.0
    config.side_vehicle_offset = 3.0
    config.front_speed_diff_pct = -60.0  # 前车更快
    config.side_speed_diff_pct = +80.0   # 左右车更慢
    config.disable_lane_change = True

    # ===== Construction场景参数 =====
    config.construction_distance = 30.0
    config.construction_length = 20.0
    config.traffic_density = 3.0
    config.traffic_speed = 8.0
    config.construction_type = "construction1"
    ...
    print("\n[1] 创建训练日志记录器...")
    logger = TrainingLogger(log_file="training_log_4scenarios.json", auto_save_interval=5)
    print("✅ 日志记录器创建成功")
    ...
    # ✅ obs_dim=30 的新权重目录
    weights_dir = "./weights/ppo-carla-4scenarios"
    load_existing = False
